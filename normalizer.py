...
from core.utils import normalize_url, host_of_url
from core.utils import maybe_fix_mojibake  # noqa: F401 (re-exported for callers that import from here)
...
_DEFAULT_TAG_RULES = [
    ...
    ("模型���布", ["gpt", "claude", "gemini", "llama", "mistral", "qwen", "deepseek", "grok", "大模型", "模型发布", "开源模型", "预训练模型", "new model", "new llm"]),
...
# 优先级：开源热榜 > 3C数码（手机/电脑硬件/数码）> AI（含原“科技”及默认）

def classify_item(record: dict[str, Any]) -> str:
    """为新闻条目分配主题分类。返回值之一：'开源热榜'/'3C数码'/'AI'。
    类目已重构：原「科技」并入「AI」大类目；原「手机/电脑硬件/数码」合并为「3C数码」。"""
    title = str(record.get("title") or "")
    source = str(record.get("source") or "")
    site_name = str(record.get("site_name") or "")
    ...
    if str(record.get("site_id") or "") == "oss_trending":
        return "开源热榜"
    # 3C数码：手机 / 电脑硬件 / 数码 关键词命中即归入
    if (
        contains_any_keyword(text, TOPIC_PHONE_KEYWORDS)
        or contains_any_keyword(text, TOPIC_HARDWARE_KEYWORDS)
        or contains_any_keyword(text, TOPIC_DIGITAL_KEYWORDS)
    ):
        return "3C数码"
    # 其余（含原“AI”、“科技”及无匹配默认）统一归入 AI 大类目
    return "AI"
